...
CREATE TABLE messages_next (
  id TEXT PRIMARY KEY,
  source TEXT NOT NULL CHECK (source IN ('hey', 'zoho', 'creative_west', 'colossal')),
...
ALTER TABLE messages ADD COLUMN discovery_context_json TEXT;

-- Metadata only: no RSS, HTML, or descriptions are retained here.
CREATE TABLE source_documents (
  id TEXT PRIMARY KEY,
  source TEXT NOT NULL,
  url TEXT NOT NULL,
  roundup_month TEXT NOT NULL,
...
CREATE INDEX idx_source_documents_pending ON source_documents(source, pending, checked_at);
CREATE TABLE source_document_messages (
...
INSERT INTO app_config(key, value) VALUES ('schema_version', '6')
