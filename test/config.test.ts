...
describe("runtime configuration", () => {
  ...
  it("enables feature flags only for the literal true value", () => {
    expect(loadRuntimeConfig(env({ NOTION_ENABLED: "true", ZOHO_ENABLED: "true", CREATIVE_WEST_ENABLED: "true", COLOSSAL_ENABLED: "true", HYPERALLERGIC_ENABLED: "true", ARTWORK_ARCHIVE_ENABLED: "true" })))
      .toMatchObject({ notionEnabled: true, zohoEnabled: true, creativeWestEnabled: true, colossalEnabled: true, hyperallergicEnabled: true, artworkArchiveEnabled: true });
    expect(loadRuntimeConfig(env({ NOTION_ENABLED: "TRUE", ZOHO_ENABLED: "1", CREATIVE_WEST_ENABLED: "TRUE", COLOSSAL_ENABLED: "1", HYPERALLERGIC_ENABLED: "1", ARTWORK_ARCHIVE_ENABLED: "TRUE" })))
      .toMatchObject({ notionEnabled: false, zohoEnabled: false, creativeWestEnabled: false, colossalEnabled: false, hyperallergicEnabled: false, artworkArchiveEnabled: false });
  });
...
describe("source labels", () => {
  it("renders human-readable source origins", () => {
    ...
    expect(sourceLabel("artwork_archive", "Opportunities")).toBe("Artwork Archive · Opportunities");
