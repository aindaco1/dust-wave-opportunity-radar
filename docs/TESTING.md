...
| HEY CLI recovery | `hey-cli-recovery.test.ts` | failed-expired-only guard; historical cutoff using zoned original timestamps; incomplete/edited-content refusal; per-message attachment ownership, size and PDF signatures; Unicode MIME; real D1/R2 importer and DOCX parsing; downstream preservation and no-write repeat; secret-free CLI help/errors |
