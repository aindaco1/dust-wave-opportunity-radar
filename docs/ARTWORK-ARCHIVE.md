...
The optional adapter reads the [Western U.S. guide filtered to Film/Video/New Media](https://www.artworkarchive.com/call-for-entry/west/guide-to-artist-grants-opportunities?opportunity_search=&opportunity_category_filter%5B%5D=5#guide-anchor). `ARTWORK_ARCHIVE_ENABLED=true` is the reviewed production setting. The September 8, 2026 Cloudflare access check succeeded; see [production acceptance](#production-acceptance--september-8-2026). An absent or non-true value disables the source. No new secret, binding, scheduler, or personal-machine service is required by this adapter. Migration 0008 adds the message source and was applied during the authorized production rollout.
...
An unauthenticated direct HTTP request using Radar's actual User-Agent returned **403, browser verification required**. Ordinary browser access succeeded. The subsequent authorized Cloudflare Worker check succeeded with two parsed entries, so the local 403 is not a current production-access blocker. For future rollouts, do not leave the source enabled for scheduled ingestion until the read-only integration check succeeds from Cloudflare. That check requires an explicitly authorized deployment with the flag enabled, inspection before the next batch slot, and disabling it again if access fails. If that access is also challenged, obtain a publisher-supported feed/API or permission for an approved retrieval method before activation. This adapter does not transfer browser cookies, solve challenges, or run a local watcher.
...
## Production acceptance — September 8, 2026

After explicit authorization, [PR #43](https://github.com/aindaco1/dust-wave-opportunity-radar/pull/43) passed required CI and merged as `52f0e2d77ad2901d935a9ffa2c92e969108f1262`. The [production workflow](https://github.com/aindaco1/dust-wave-opportunity-radar/actions/runs/34224590934) passed all 363 tests and the complete gate, applied migration 0008 at 12:10:54 UTC, and deployed Worker version `1c6d5be2-e2b2-4d90-be77-1dc5f9c7054c` at 12:11:03 UTC (06:11 Mountain Time).

[Read-only integration verification](https://github.com/aindaco1/dust-wave-opportunity-radar/actions/runs/34224708336) succeeded at 12:11:46 UTC. Artwork Archive returned `ok: true`, `matchingEntries: 2`, and `skipped: false`; every integration check passed. The earlier local browser-verification 403 did not reproduce from Cloudflare. Live health confirmed `artworkArchiveEnabled: true`, all prior flags unchanged, and the existing 07:00/19:00 America/Denver schedule.

Independent remote D1 inspection confirmed schema version 8 and the migration record. Pre/post deployment counts matched: 544 messages, 86 opportunities, 131 digest items, 83 runs, four source documents, and 92 document/message links. Managed-Markdown coverage remained 84 of 84 managed Notion mappings; the manual-mapping count remained two. The foreign-key check was clean before and after.

No source-only import, forced batch, Notion write, digest send, secret change, or Email Routing change was performed. The integration check parsed the live guide without storing candidates. The source remains enabled for the next regular batch at 07:00 Mountain Time. That batch's ingestion, classification, Notion, and digest outcomes have not yet been verified by this acceptance.
