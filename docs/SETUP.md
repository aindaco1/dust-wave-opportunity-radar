...
See the [HEY CLI runbook](HEY-CLI.md) for the pinned build and current recovery procedure, its [qualification history](HEY-CLI.md#qualification-and-acceptance-history) for the dated attachment, identity, and hosted checks, and [future-expansion boundaries](HEY-CLI.md#boundaries-for-any-future-expansion) for the unresolved forwarding-overlap and unattended-authentication scope. Do not copy `hey auth token` output into logs or the repository.
