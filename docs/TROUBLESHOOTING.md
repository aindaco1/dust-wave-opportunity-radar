...
For one existing failed topic with expired raw data, prefer [guarded official-CLI recovery](HEY-CLI.md#scoped-historical-recovery). Preview first, authorize the source-only import explicitly, verify the same D1 identity and no-write repeat, and delete its temporary token. This path rejects new identities, edited older messages, retained-failed records, and missing attachments. If a failed run reports `writeAttempted: true`, inspect D1 before retrying; do not interpret a network error as proof the import did not happen. The legacy `HEY_BACKFILL_TARGETS_JSON` mechanism remains available but does not provide these stricter recovery checks.
