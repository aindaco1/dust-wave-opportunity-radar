...
- Existing failed HEY topic with expired raw content: use **Recover one historical HEY record**, initially in `preview` mode, with its existing D1 hash and a temporary `HEY_CLI_RECOVERY_TOKEN` in the protected `production` environment. `import` requires explicit production authorization; it restores only pre-original-import messages and refuses edited historical content or incomplete attachments. Verify `recovered`, `identityCount: 1`, and `repeatImported: 0`; remove the temporary secret on success or failure. This does not trigger classification/Notion/digest. If `writeAttempted` is true on a failed run, reconcile D1 before retrying. Never concurrently edit/delete the target. See [scoped recovery](HEY-CLI.md#scoped-historical-recovery).
